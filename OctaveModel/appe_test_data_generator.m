...
fprintf(fp, "const uint32_t appePrimDecodingLUT[%d] =\n{\n", lut_elements+1);
for i=1:lut_rows
...
fprintf(fp, "const uint32_t appeBackDecodingLUT[%d] =\n{\n", lut_elements+1);
for i=1:lut_rows
  for j=1:lut_row_elements
    fprintf(fp, "%4d", lut_adc(1,(i-1)*lut_row_elements + j));
    fprintf(fp, ", ");
...
fp = fopen ("appe_decoder.h", 'w+');
fprintf(fp, "/*file generated with %s.m octave/matlab script %s*/\n\n", mfilename(), datestr(clock()));
fprintf(fp, "#include <stdint.h>\n");
fprintf(fp, "#ifndef APPE_DECODER_H\n");
fprintf(fp, "#define APPE_DECODER_H\n\n");
fprintf(fp, "#define APPE_DECODER_LUT_ELEMENTS  %d\n\n", lut_elements+1);
fprintf(fp, "extern const uint32_t appePrimDecodingLUT[];\n");
fprintf(fp, "extern const uint32_t appeBackDecodingLUT[];\n\n");
fprintf(fp, "#endif /*APPE_DECODER_H*/\n");
...
csvwrite('appe_test_data.csv', input_data);
fp = fopen('appe_test_data.csv', 'a+');
fprintf(fp, "\n");
fclose(fp);
